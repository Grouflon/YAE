...
#define TEST(_cond) if (!(_cond)) { throw(#_cond); }

void runTest(const char* _testName, void(*_testFunction)())
{
	try
	{
		_testFunction();
		YAE_LOGF_CAT("test", "%s: SUCCESS", _testName);
	}
	catch(const char* _e)
	{
		YAE_ERRORF_CAT("test", "%s: FAILED (%s)", _testName, _e != nullptr ? _e : "");
	}
}
...
{
	YAE_LOG_CAT("test", "Start unit tests");

	runTest("JsonSerializer", &serialization::testJsonSerializer);
	runTest("BinarySerializer", &serialization::testBinarySerializer);

	YAE_LOG_CAT("test", "End unit tests");
}
