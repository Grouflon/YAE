...
static VmaAllocation            g_FontMemory = VK_NULL_HANDLE;
static VkImage                  g_FontImage = VK_NULL_HANDLE;
static VkImageView              g_FontView = VK_NULL_HANDLE;
static VmaAllocation            g_UploadBufferMemory = VK_NULL_HANDLE;
...
void ImGui_ImplVulkanH_DestroyFrameRenderBuffers(VkDevice device, VmaAllocator vmaAllocator, ImGui_ImplVulkanH_FrameRenderBuffers* buffers, const VkAllocationCallbacks* allocator);
void ImGui_ImplVulkanH_DestroyWindowRenderBuffers(VkDevice device, VmaAllocator vmaAllocator, ImGui_ImplVulkanH_WindowRenderBuffers* buffers, const VkAllocationCallbacks* allocator);
...
static void CreateOrResizeBuffer(VkBuffer& buffer, VmaAllocation& buffer_memory, VkDeviceSize& p_buffer_size, size_t new_size, VkBufferUsageFlagBits usage)
{
    ImGui_ImplVulkan_InitInfo* v = &g_VulkanInitInfo;
    VkResult err;
    ...
    if (buffer_memory != VK_NULL_HANDLE)
        vmaFreeMemory(v->VmaAllocator, buffer_memory);
    ...
    err = vmaAllocateMemory(v->VmaAllocator, &req, &allocInfo, &buffer_memory, nullptr);
    check_vk_result(err);

    err = vmaBindBufferMemory(v->VmaAllocator, buffer_memory, buffer);
    check_vk_result(err);
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        VkResult err = vmaMapMemory(v->VmaAllocator, rb->VertexBufferMemory, (void**)(&vtx_dst));
        check_vk_result(err);
        err = vmaMapMemory(v->VmaAllocator, rb->IndexBufferMemory, (void**)(&idx_dst));
        check_vk_result(err);
        ...
        err = vmaFlushAllocations(v->VmaAllocator, 2, allocations, offsets, sizes);
...
bool ImGui_ImplVulkan_CreateFontsTexture(VkCommandBuffer command_buffer)
{
    ...
    // Create the Image:
    {
        ...
        err = vmaAllocateMemory(v->VmaAllocator, &req, &alloc_info, &g_FontMemory, nullptr);
        check_vk_result(err);
        err = vmaBindImageMemory(v->VmaAllocator, g_FontMemory, g_FontImage);
        check_vk_result(err);
    }
    ...
    // Create the Upload Buffer:
    {
        ...
        err = vmaAllocateMemory(v->VmaAllocator, &req, &alloc_info, &g_UploadBufferMemory, nullptr);
        check_vk_result(err);
        err = vmaBindBufferMemory(v->VmaAllocator, g_UploadBufferMemory, g_UploadBuffer);
        check_vk_result(err);
    }
    ...
    {
        char* map = NULL;
        err = vmaMapMemory(v->VmaAllocator, g_UploadBufferMemory, (void**)(&map));
        check_vk_result(err);
        memcpy(map, pixels, upload_size);
        err = vmaFlushAllocation(v->VmaAllocator, g_UploadBufferMemory, 0, upload_size);
        check_vk_result(err);
        vmaUnmapMemory(v->VmaAllocator, g_UploadBufferMemory);
    }
...
void    ImGui_ImplVulkan_DestroyFontUploadObjects()
{
    ...
    {
        vmaFreeMemory(v->VmaAllocator, g_UploadBufferMemory);
...
{
    ImGui_ImplVulkan_InitInfo* v = &g_VulkanInitInfo;
    ImGui_ImplVulkanH_DestroyWindowRenderBuffers(v->Device, v->VmaAllocator, &g_MainWindowRenderBuffers, v->Allocator);
    ...
    if (g_FontMemory)           { vmaFreeMemory(v->VmaAllocator, g_FontMemory); g_FontMemory = VK_NULL_HANDLE; }
...
void ImGui_ImplVulkanH_DestroyFrameRenderBuffers(VkDevice device, VmaAllocator vmaAllocator, ImGui_ImplVulkanH_FrameRenderBuffers* buffers, const VkAllocationCallbacks* allocator)
{
    ...
    if (buffers->VertexBufferMemory) { vmaFreeMemory(vmaAllocator, buffers->VertexBufferMemory); buffers->VertexBufferMemory = VK_NULL_HANDLE; }
    if (buffers->IndexBuffer) { vkDestroyBuffer(device, buffers->IndexBuffer, allocator); buffers->IndexBuffer = VK_NULL_HANDLE; }
    if (buffers->IndexBufferMemory) { vmaFreeMemory(vmaAllocator, buffers->IndexBufferMemory); buffers->IndexBufferMemory = VK_NULL_HANDLE; }
...
void ImGui_ImplVulkanH_DestroyWindowRenderBuffers(VkDevice device, VmaAllocator vmaAllocator, ImGui_ImplVulkanH_WindowRenderBuffers* buffers, const VkAllocationCallbacks* allocator)
{
    for (uint32_t n = 0; n < buffers->Count; n++)
        ImGui_ImplVulkanH_DestroyFrameRenderBuffers(device, vmaAllocator, &buffers->FrameRenderBuffers[n], allocator);
